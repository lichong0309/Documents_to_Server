...
#include <time.h>
struct timespec start_init_enclave, stop_init_enclave, start_1, stop_1, start_2, stop_2, start, stop;                        // 使用clock_gettime()函数，存储当前的时间
double diff;
...
int SGX_CDECL main(int argc, char *argv[])
{
    ...
    clock_gettime(CLOCK_MONOTONIC_RAW, &start_init_enclave);                                 // 获得当前时钟，存放在start_init_enclave中，记作为测试开始的时钟;   
    /* Initialize the enclave */
    ...
    clock_gettime(CLOCK_MONOTONIC_RAW, &stop_init_enclave);                                      //  获取当前时钟，存放到stop_init_enclave中，就作为测试结束的时钟;
    printf("Total init enclave time: %f mins\n", time_diff(&start_init_enclave, &stop_init_enclave, SEC) / 60);                          // 打印测试数据所需要的时间;
    ...
    clock_gettime(CLOCK_MONOTONIC_RAW, &start_1);                                 // 获得当前时钟，存放在start_1中，记作为测试开始的时钟;
     // 测试数据,神经网络配置文件cfg = MNIST_CFG= "./App/dnet-out/cfg/alexnet.cfg";
     // 返回中间层的返回值；           
    float *middle_layer_output = test_mnist(cfg);                                            
